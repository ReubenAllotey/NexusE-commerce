...
function mapPaymentReceipt(paymentRow = {}, orderRow = null, items = []) {
  ...
  return {
    ...
    items: items.map((item) => ({
      ...
      shippingFeeStatus: clean(item.shipping_fee_status),
      shippingPaidAmount: Number(item.shipping_paid_amount) || 0,
  ...
  const { data: itemRows, error: itemError } = await supabase
    ...
    .select(
      [
        ...
        "selected_color",
        "selected_size",
        "shipping_fee,shipping_fee_status,shipping_paid_amount",
        "line_subtotal",
        "line_shipping",
        "created_at",
      ].join(","),
