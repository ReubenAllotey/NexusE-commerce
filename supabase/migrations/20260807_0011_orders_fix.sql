...
begin
  ...
  loop
    v_items_json := v_items_json || jsonb_build_array(
      jsonb_build_object(
        'id', v_row.cart_item_id,
        'orderId', null,
        'productId', v_row.product_id,
        'productName', v_row.product_name,
        'productSlug', v_row.product_slug,
        'brand', v_row.brand,
        'imageUrl', v_row.primary_image_url,
        'unitPrice', v_row.unit_price,
        'quantity', v_row.quantity,
        'selectedColor', v_row.selected_color,
        'selectedSize', v_row.selected_size,
        'shippingFee', v_row.shipping_fee,
        'lineSubtotal', v_row.unit_price * v_row.quantity,
        'lineShipping', v_row.shipping_fee * v_row.quantity,
        'createdAt', v_row.created_at
      )
    );

    v_subtotal := v_subtotal + (v_row.unit_price * v_row.quantity);
    v_shipping_total := v_shipping_total + (v_row.shipping_fee * v_row.quantity);

    if v_shipment_type is null then
      v_shipment_type := v_row.normalized_shipping_method;
    elsif v_shipment_type <> v_row.normalized_shipping_method then
      v_shipment_type := 'both';
    end if;
  end loop;
  ...
  select
    ...
    nullif(btrim(item ->> 'selectedColor'), ''),
    nullif(btrim(item ->> 'selectedSize'), ''),
    coalesce((item ->> 'shippingFee')::numeric, 0),
    (item ->> 'lineSubtotal')::numeric,
    (item ->> 'lineShipping')::numeric,
