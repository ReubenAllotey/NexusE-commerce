...
-- Preserve the product shipping snapshot at checkout. Historical rows remain
-- nullable because their original pending/ready state cannot be inferred safely.
alter table public.order_items
  add column if not exists shipping_fee_status text
  constraint order_items_shipping_fee_status_check
  check (shipping_fee_status in ('pending', 'ready')),
  add column if not exists shipping_paid_amount numeric
  constraint order_items_shipping_paid_amount_check
  check (shipping_paid_amount is null or shipping_paid_amount >= 0);

-- `total` remains the known commercial order total. This separate field is the
-- amount that the current checkout should send to Paystack.
alter table public.orders
  add column if not exists shipping_payment_mode text
  constraint orders_shipping_payment_mode_check
  check (shipping_payment_mode in ('pay_now', 'pay_later')),
  add column if not exists shipping_payment_status text
  constraint orders_shipping_payment_status_check
  check (shipping_payment_status in ('not_due', 'pending', 'outstanding', 'partial', 'paid')),
  add column if not exists amount_due_now numeric
  constraint orders_amount_due_now_check
...
begin
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'shippingFeeStatus', oi.shipping_fee_status,
        'shippingPaidAmount', oi.shipping_paid_amount,
  ...
  return jsonb_build_object(
    'order', jsonb_build_object(
      ...
      'total', v_order.total,
      'amountDueNow', v_order.amount_due_now,
      'shippingPaymentMode', v_order.shipping_payment_mode,
      'shippingPaymentStatus', v_order.shipping_payment_status,
...
declare
  ...
  v_ready_amount_due_now numeric := 0;
  v_preorder_amount_due_now numeric := 0;
  v_ready_shipping_payment_status text := 'not_due';
  v_preorder_shipping_payment_status text := 'not_due';
...
begin
  ...
  v_shipping_payment_mode := lower(btrim(coalesce(
    v_payload ->> 'shippingPaymentPreference',
    v_payload ->> 'shipping_payment_mode',
    'pay_now'
  )));

  if v_shipping_payment_mode not in ('pay_now', 'pay_later') then
    raise exception 'Unsupported shipping payment preference.';
  end if;
  ...
  select
    ...
    case
      when lower(coalesce(p.shipping_fee_status, 'pending')) = 'ready'
        and p.shipping_fee is not null then p.shipping_fee
      else 0
    end as shipping_fee,
    case
      when lower(coalesce(p.shipping_fee_status, 'pending')) = 'ready'
        and p.shipping_fee is not null then 'ready'
      else 'pending'
    end as shipping_fee_status,
    case
  ...
  left join lateral (
    ...
    join public.product_variation_options as o on true
    join public.product_variation_groups as g
      on g.id = o.group_id and g.product_id = p.id
    where (
      (
      ...
      or (
        ...
        and (nullif(selected.value ->> 'groupId', '') is null or g.id = nullif(selected.value ->> 'groupId', '')::uuid)
        and (nullif(selected.value ->> 'group_id', '') is null or g.id = nullif(selected.value ->> 'group_id', '')::uuid)
        and (nullif(selected.value ->> 'groupName', '') is null or lower(g.group_name) = lower(nullif(selected.value ->> 'groupName', '')))
        and (nullif(selected.value ->> 'group_name', '') is null or lower(g.group_name) = lower(nullif(selected.value ->> 'group_name', '')))
        and (
          (
            coalesce(nullif(selected.value ->> 'label', ''), nullif(selected.value ->> 'option_label', '')) is not null
            and lower(o.option_label) = lower(coalesce(nullif(selected.value ->> 'label', ''), nullif(selected.value ->> 'option_label', '')))
          )
          or (
            coalesce(nullif(selected.value ->> 'value', ''), nullif(selected.value ->> 'option_value', '')) is not null
            and lower(coalesce(o.option_value, '')) = lower(coalesce(nullif(selected.value ->> 'value', ''), nullif(selected.value ->> 'option_value', '')))
          )
        )
      )
    )
    order by
      case when o.id = nullif(selected.value ->> 'optionId', '')::uuid
        or o.id = nullif(selected.value ->> 'option_id', '')::uuid then 0 else 1 end,
  ...
  select
    ...
    coalesce(sum(shipping_fee * quantity) filter (where availability_type = 'ready_stock' and shipping_fee_status = 'ready'), 0),
    coalesce(sum(unit_price * quantity) filter (where availability_type = 'preorder'), 0),
    coalesce(sum(shipping_fee * quantity) filter (where availability_type = 'preorder' and shipping_fee_status = 'ready'), 0)
  into v_ready_subtotal, v_ready_shipping, v_preorder_subtotal, v_preorder_shipping
  from pg_temp.nexus_checkout_rows;

  v_ready_amount_due_now := v_ready_subtotal + case when v_shipping_payment_mode = 'pay_now' then v_ready_shipping else 0 end;
  v_preorder_amount_due_now := v_preorder_subtotal + case when v_shipping_payment_mode = 'pay_now' then v_preorder_shipping else 0 end;

  select case
    when count(*) filter (where availability_type = 'ready_stock' and shipping_fee_status = 'pending') > 0
      and coalesce(sum(shipping_fee * quantity) filter (where availability_type = 'ready_stock' and shipping_fee_status = 'ready'), 0) = 0
      then 'pending'
    when coalesce(sum(shipping_fee * quantity) filter (where availability_type = 'ready_stock' and shipping_fee_status = 'ready'), 0) > 0
      then 'outstanding'
    else 'not_due'
  end,
  case
    when count(*) filter (where availability_type = 'preorder' and shipping_fee_status = 'pending') > 0
      and coalesce(sum(shipping_fee * quantity) filter (where availability_type = 'preorder' and shipping_fee_status = 'ready'), 0) = 0
      then 'pending'
    when coalesce(sum(shipping_fee * quantity) filter (where availability_type = 'preorder' and shipping_fee_status = 'ready'), 0) > 0
      then 'outstanding'
    else 'not_due'
  end
  into v_ready_shipping_payment_status, v_preorder_shipping_payment_status
  from pg_temp.nexus_checkout_rows;
  ...
  if v_has_ready_stock then
    ...
    insert into public.orders (
      ...
      total, shipping_payment_mode, shipping_payment_status, amount_due_now, estimated_arrival, preorder_terms,
      ...
      v_checkout_group_id, v_address.id, v_shipping_address_snapshot,
      v_ready_subtotal, v_ready_shipping, v_ready_subtotal + v_ready_shipping,
      v_shipping_payment_mode, v_ready_shipping_payment_status, v_ready_amount_due_now, null, null, now(), now()
    ...
    insert into public.order_items (
      ...
      shipping_fee, shipping_fee_status, shipping_paid_amount, availability_type, estimated_arrival, preorder_terms,
    ...
    select
      ...
      shipping_fee, shipping_fee_status, 0, availability_type, null, null,
      unit_price * quantity, case when shipping_fee_status = 'ready' then shipping_fee * quantity else 0 end, created_at
    from pg_temp.nexus_checkout_rows
    where availability_type = 'ready_stock';

    v_ready_order := jsonb_set(private.get_order_bundle(v_order.id), '{order,checkoutGroupId}', to_jsonb(v_checkout_group_id), true);
    v_orders := v_orders || jsonb_build_array(v_ready_order);
  end if;

  if v_has_preorder then
    select
      case when count(*) filter (where estimated_arrival is not null) = 0 then null
        else (array_agg(estimated_arrival order by created_at asc, cart_item_id asc) filter (where estimated_arrival is not null))[1] end,
      case when count(*) filter (where preorder_terms is not null) = 0 then null
        else (array_agg(preorder_terms order by created_at asc, cart_item_id asc) filter (where preorder_terms is not null))[1] end
    ...
    insert into public.orders (
      ...
      v_checkout_group_id, v_address.id, v_shipping_address_snapshot,
      v_preorder_subtotal, v_preorder_shipping, v_preorder_subtotal + v_preorder_shipping,
      v_shipping_payment_mode, v_preorder_shipping_payment_status, v_preorder_amount_due_now, v_estimated_arrival, v_preorder_terms,
      now(), now()
    ) returning * into v_order;
    ...
    select
      v_order.id, product_id, product_name, product_slug, brand, primary_image_url,
      unit_price, quantity, selected_color, selected_size, variant_key, selected_options,
      shipping_fee, shipping_fee_status, 0, availability_type, estimated_arrival, preorder_terms,
      unit_price * quantity, case when shipping_fee_status = 'ready' then shipping_fee * quantity else 0 end, created_at
    from pg_temp.nexus_checkout_rows
    where availability_type = 'preorder';

    v_preorder_order := jsonb_set(private.get_order_bundle(v_order.id), '{order,checkoutGroupId}', to_jsonb(v_checkout_group_id), true);
    v_orders := v_orders || jsonb_build_array(v_preorder_order);
  end if;

  delete from public.cart_items as ci where ci.cart_id = v_cart.id;
  update public.carts as c set updated_at = now() where c.id = v_cart.id;
  ...
  return jsonb_build_object(
    ...
    'combined', jsonb_build_object(
      ...
      'total', v_ready_subtotal + v_ready_shipping + v_preorder_subtotal + v_preorder_shipping,
      'amountDueNow', v_ready_amount_due_now + v_preorder_amount_due_now,
      'shippingPaymentMode', v_shipping_payment_mode,
      'shippingPaymentStatus', jsonb_build_object(
        'readyStock', v_ready_shipping_payment_status,
        'preorder', v_preorder_shipping_payment_status
      )
    ),
